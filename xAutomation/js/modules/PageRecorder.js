	...
	(function(AutomationFront){
		var isLinked = false, showCursor, showElementSelector = false, stopSendingActions = false, selectedElement, selectedRect, elementSelector;
		...
		function createElementSelectorBorder() {
			var div = document.createElement('div');
		...
		function setRect(div, x, y, width, height) {
			div.style.left = x + 'px';
			div.style.top = y + 'px';
			div.style.width = width + 'px';
			div.style.height = height + 'px';
		...
		function init() {
			if(!isLinked) {
				...
				elementSelector = createElementSelectorBorder();
				
				document.addEventListener('mousemove', function(e) {
					var position;
					
					if(showCursor) {
						chrome.runtime.sendMessage(new Messaage('front-cursor-iframe', getFrameIndex()));
		...
		(function(simulate) {
			...
			function mouseEvent(event, x, y, key) {
				var eventDispatcher = document.createEvent("MouseEvent");
				var domElement = document.elementFromPoint(x, y);
				eventDispatcher.initMouseEvent(event, true, true, window, null, x, y, x, y, false, false, false, false, key, null);
				
				domElement.dispatchEvent(eventDispatcher);
			}
